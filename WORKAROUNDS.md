...
- **Exit condition**: The operator-UX modules (`modules/shell/utilities.nix` toolbox, `modules/shell/common.nix` bash baseline, `modules/shell/prompt.nix` PS1, `modules/motd/default.nix`, `modules/users/operator.nix`, home-manager modular split) are reintroduced one at a time via canary deploys per [plan.md → Phase 5](./specs/001-nixos-rpi-cluster/plan.md), each with a smoke-test gate, on all 9 work-set nodes green. SSH hardening (W-003 close) is the final step.
- **Target phase / feature**: This spec's Phase 5 (Operator UX). Each module sub-step closes the inline-host approach for that concern; the entry is fully Resolved when Phase 5 exits with all work-set nodes on the modular configuration.
- **Opened**: 2026-04-26
- **Resolved**: (open)
