...
The operator's gaming peripherals work under NixOS: Xbox One controllers connect via USB or Bluetooth, Logitech joysticks are recognized as input devices, and the Logitech G29 racing wheel provides force feedback in games via Steam/Proton.
...
- **FR-008**: System MUST support Xbox One controllers, primarily over USB (wired) with Bluetooth as a secondary option. Phase 5 MUST first verify whether the kernel-native xpad driver provides full support (USB + Bluetooth) on both Gibson and Silicon. xpadneo is added to shared gaming.nix only if native xpad is insufficient; if native xpad works fully, xpadneo is not required
...
- **FR-018**: System MUST support 5.1 surround audio output (analog multi-channel via motherboard's 3 audio jacks: front, rear, center/sub) and microphone input via onboard motherboard audio through PipeWire with the appropriate surround profile enabled. Configuration lives in `modules/hardware/gibson.nix` (FR-012)
...
- **FR-026**: *(Constitution Principle IX enforcement)* Silicon's evaluated NixOS configuration MUST NOT change at any point during this spec. Every commit that touches shared modules, import paths, or flake-level config MUST be verified with `make dry-run HOST=silicon` producing an identical store path. Zero architectural changes to Silicon — no renderer swaps, no default changes, no "while we're in here" cleanups. **Exception — Phase 5 peripheral enablement**: Changes to `modules/nixos/workstation/gaming.nix` that add kernel module support (new-lg4ff, and xpadneo if needed per FR-008) MAY change Silicon's derivation. Both hosts use Xbox One controllers — if xpadneo is required (native xpad insufficient), adding it to shared gaming.nix is intentional standardization benefiting both hosts. Operator MUST visually validate Silicon after applying. If derivation change is unacceptable, refactor to Gibson-only module
...
- **FR-031**: Phase structure MUST be: Phases 1–6 as planned, Phase 7 = code deduplication, Phase 8 = polish (formerly Phase 7). Phase 7 infrastructure work (hostProfile NixOS options) MAY run in parallel with Phases 4–6. Phase 7 i3 and polybar deduplication require Phase 4 completion (can't diff files not yet customized for Gibson). Phase 7 dedup work MUST NOT block Gibson-specific work in Phases 4–6
...
- Q: Wallpaper? → A: Reorganize `assets/` into per-host directories (`assets/laptop/`, `assets/gibson/`). Current assets (wallpaper-gibson.png, wallpaper-gibson-blur.png, login.png) are laptop-specific — move to `assets/laptop/`. Copy as starting point for `assets/gibson/`. Fresh Gibson wallpapers made later. `assets/wallpaper.png` is unused, can be removed
...
- Q: Silicon also uses Xbox One controller — is xpadneo actually needed? → A: Both hosts use Xbox One controllers. Silicon currently uses kernel-native xpad only. Phase 5 must first verify whether native xpad provides full support (USB + Bluetooth) on both hosts. xpadneo added to shared gaming.nix only if native xpad is insufficient — in that case it's intentional standardization benefiting both hosts. If native xpad works fully, xpadneo is not required at all.
