...
- **Two-phase provision: stage3 failure** — `provision-stage2` (minimal install) succeeds and the node reboots, but `provision-stage3` (`update-node` with full config) fails. The node is running on the provision-minimal config: bootable, SSH-reachable as `bob`, but missing home-manager, toolbox, MOTD, and PS1 customization. Recovery: operator retries `make provision-stage3 HOST=<host>` (or `make update-node HOST=<host>` directly). No re-provisioning required — the node's disk layout and RAID are intact.
- **Two-phase provision: stage2 failure (disko succeeded)** — `provision-stage1` (disko) formats the RAID successfully, but `provision-stage2` (nixos-anywhere install) fails or times out before completing. The node reboots to the SD baseline. The RAID array exists but has no NixOS installed. Normal `make provision` refuses (FR-013: RAID detected). Recovery: operator runs `make provision-reinstall HOST=<host>`, which skips disko (RAID already formatted), mounts the existing array, and runs the install + full config push. Pre-flight distinguishes this state from a fully provisioned node by checking that RAID exists in `/proc/mdstat` but root is NOT md-backed (booted from SD).
...
- **FR-006**: Top-level per-host `flake.nix` entries MUST include only minimal inclusions (per-host `configuration.nix`, cluster-specific configuration, device-specific configuration); deeper modules MUST be imported from within the appropriate scope-level module rather than enumerated at the host level. The cluster module imported by each host config MAY be injected via `specialArgs` (with a default) to support build variants (e.g., provision-minimal vs. full); the builder selects the variant, not the host config.
...
- **FR-011**: The boot order MUST prefer the USB array when healthy and fall back to the SD-card recovery environment when the USB array is absent or unbootable. Recovery from SD MUST be possible without operator intervention beyond removing/replacing USB drives. The initrd MUST include a guided recovery tool (`hlc-recover`) accessible via dropbear SSH when the RAID fails to assemble. The provisioning workflow MUST backup the SD bootstrap boot files (`provision-backup-boot`) before overwriting them so that `hlc-recover sd-boot` can restore the bootstrap without requiring a full SD reflash.
- **FR-012**: The provisioning workflow (`nixos-anywhere` invocation + `disko` schema) MUST be invokable from `gibson` or `silicon` against a SD-baseline node. Operator MAY also invoke it on the node itself; both paths MUST be documented. `nixos-anywhere` MUST connect as `bob`; this requires `bob` to have passwordless sudo on the bootstrap image. No explicit sudo flag is needed on the `nixos-anywhere` invocation — passwordless sudo on the target is sufficient (confirmed 2026-05-05). Provisioning MUST use a **two-phase approach**: (1) `nixos-anywhere` installs a **provision-minimal** NixOS configuration — sufficient to boot from USB RAID, run SSH as `bob`, and accept `nix copy` — but excluding home-manager, the sysadmin toolbox, MOTD, PS1 customization, and static `/etc/hosts` entries; (2) after reboot and a successful `smoke-test`, `update-node` pushes the full per-host configuration as a differential `nix copy`. The provision-minimal configuration MUST be a separate flake output (`<host>-provision`) built by a dedicated builder that injects a minimal cluster module via `specialArgs`. If stage 2 (`update-node`) fails, the node remains SSH-reachable on the minimal config; the operator retries `make provision-stage3` or `make update-node` without re-provisioning.
...
- **FR-017**: A stylized HLC-themed PS1 MUST be defined for cluster nodes, using the Bob Ross-themed glyph sequence `☁⛰☁` (cloud, mountain, cloud) as the HLC motif. The PS1 is a single unified form for all contexts (local and remote) — a two-line, box-drawing prompt with Gruvbox 256-color palette (109=teal, 214=gold, 142=green, 15=bright white):
      ...
      └──╸$
      ```
      where `<hostname>` is the short hostname (e.g. `bob@hlc-501`; `\H` returns the kernel hostname) and `<cwd>` appears in square brackets. Box-drawing glyphs and brackets are teal; username and hostname are gold; `@` and `$` are bright white; working directory is green. The PS1 MUST be set via `programs.bash.promptInit` (not profile.d) to avoid being overwritten by NixOS default prompt initialization.
...
- **FR-018**: Each cluster node MUST display a per-host MOTD on SSH login showing the HLC ASCII banner, `Cluster node: <fqdn>`, and Bob Ross quote. The MOTD MUST be styled with the Gruvbox 256-color palette: gold borders/punctuation (`"`, `:`, `~`), teal ASCII art and quote text, green "Happy Little Cloud", red "Bob Ross", bright white hostname. The hostname MUST be interpolated dynamically from `config.networking.fqdn`; the banner and quote are static content with inline ANSI color codes. The MOTD MUST be implemented as a parameterized NixOS module (`cluster.motd.{banner,quote}`) so future clusters can override the banner.
...
- **FR-024**: The operator workflow MUST expose, at minimum, the following Makefile (or equivalent) targets by end of spec. Each target is added in the phase that first needs it (just-in-time, not pre-built); the set below is the end-of-spec total, not the Phase 0 starting state:
  ...
  - `provision HOST=<host>` — two-phase provisioning: `provision-stage1` (disko), `provision-mount` (firmware), `provision-stage2` (install provision-minimal config via `nixos-anywhere`), `smoke-test` (verify SSH reachability after reboot, clears stale host keys), `provision-stage3` (push full config via `update-node`), `smoke-test` (verify full config).
  - `provision-stage3 HOST=<host>` — wait for node reboot, then push full config via `update-node`. Retriable independently on failure.
  - `provision-reinstall HOST=<host>` — RAID-retry path for failed stage2 where disko succeeded but install failed/timed out. Skips disko, mounts existing RAID, runs install + full config push. Pre-flight requires RAID to exist but root NOT md-backed (distinguishes failed-install from fully-provisioned node).
  - `update-node HOST=<host>` — single-node local build + `nix copy` + remote activate for post-provisioning configuration updates.
  - `rollback HOST=<host>` — single-node `nixos-rebuild --rollback --target-host`.

  - `recover HOST=<host>` — full recovery cycle for a node stuck in initrd rescue: wipe RAID, restore SD bootstrap boot, reboot, wait for SD baseline, then run full provision.
  - `recover-status HOST=<host>` — query initrd rescue node state via `hlc-recover status` (root SSH to dropbear).
  - `recover-wipe HOST=<host>` — wipe RAID on an initrd rescue node via `hlc-recover wipe`.
  - `recover-sd-boot HOST=<host>` — restore SD bootstrap boot files on an initrd rescue node via `hlc-recover sd-boot`.
  - `provision-backup-boot HOST=<host>` — backup SD firmware partition contents before `provision-stage2` overwrites them with provisioned NixOS boot files. Enables `hlc-recover sd-boot` without reflashing.
...
- Note (hardware status as of 2026-05-08): All work-set nodes online and provisionable except hlc-503 and hlc-507 (hardware issues; tracked in Makefile `DECOM_HOSTS`). All online Pi 5 nodes confirmed: mdadm array at `/dev/md127` mounted as `/`, NVMe present and confirmed working. hlc-401 online and ready. Phase 5 gate may be tagged with documented exceptions for nodes tracked in `DECOM_HOSTS`; those nodes are provisioned once hardware issues resolve. *Update 2026-05-18*: hlc-503 board physically replaced; node provisioned and online (W-015 resolved). hlc-507 remains DECOM (W-016 open).
...
- Q20: Two-phase provision — new FR or amend existing? → A: Amend FR-012 and FR-024 inline. This is a refinement of the provisioning workflow, not a new capability. Resolved in FR-012 and FR-024.
- Q21: Two-phase provision — stage3 failure recovery path? → A: Operator retries `make provision-stage3` or `make update-node` directly. Node remains SSH-reachable on the minimal config; no re-provisioning required. Resolved in Edge Cases and FR-012.
- Q22: Should smoke-test gate between provision stages? → A: Yes. Composite `provision` target runs: stage1 → mount → stage2 → smoke-test → stage3 → smoke-test. The mid-provision smoke-test also clears stale SSH host keys from `known_hosts`, preventing key-mismatch failures in stage3. Resolved in FR-024.
- Q23: FR-006 amendment for specialArgs cluster module injection? → A: Deduplicate into existing FR-006 wording. The cluster module MAY be injected via specialArgs with a default to support build variants. Resolved in FR-006.
- Q24: SC-001 — does 30-minute target include stage3 (full config push)? → A: Yes. SC-001 means fully provisioned through stage3. If the 30-minute target proves tight, a sub-target for stage2 SSH reachability may be added. Resolved in SC-001 (no text change needed — "SSH-reachable" already implies full provision since two-phase is transparent to the success criterion).
