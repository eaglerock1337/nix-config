...
- Q: Does SD bootstrap `bob` need passwordless sudo for nixos-anywhere? → A: Yes, already configured. `bob` has passwordless sudo in the bootstrap image (`modules/sd/bootstrap.nix`). `nixos-anywhere` is invoked with `--use-remote-sudo bob@<host>`. This is bootstrap-scoped only; full per-host sudo policy is independent. Resolved in FR-012.
