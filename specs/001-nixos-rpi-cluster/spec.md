...
- Q8: Phase rollout cadence — per-module canary or phase bundle? → A: **Phase bundle** (Option B). `/speckit-implement` runs all module-creation tasks within a phase, then a single canary on `hlc-501` with the full bundle. Smoke-test green → fleet roll. On smoke-test fail → rollback + incremental validation (comment out / git-bisect new modules, reintroduce one at a time) per the `/speckit-debug` skill. Requires downstream amendments: W-001 ledger admits phase-bundle scope; Constitution IV PATCH bump clarifies canary scope = bundle (not strictly per-module); plan Phase 5 phasing notes updated; tasks.md US-phase rollout sections restructured (single canary per phase, not per module). FR-024's manual-canary procedure (`update-node` → `smoke-test` → manual `rollback`) still applies; only the *scope* of one canary expands from one module to one bundle.
