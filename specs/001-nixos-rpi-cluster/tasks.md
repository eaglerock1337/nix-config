...
**Purpose**: Confirm current repo state is correct before any hardware work.
...
**Purpose**: Prove Pi4 and Pi5 minimal configs boot and accept SSH.

**⚠️ CRITICAL**: Both canaries must pass smoke-test before Phase 3 begins.
...
- [ ] T006 Flash hlc-401 SD: `make flash-image HOST=hlc-401 DEV=<dev>`; insert SD, power on, wait for DHCP
- [ ] T007 [P] Flash hlc-501 SD: `make flash-image HOST=hlc-501 DEV=<dev>`; insert SD, power on, wait for DHCP
- [ ] T008 Smoke-test Pi4 canary: `make smoke-test HOST=hlc-401 IP=10.23.50.41`; verify default bash prompt on interactive SSH (kitty users: `TERM=xterm-256color ssh bob@10.23.50.41`)
- [ ] T009 Smoke-test Pi5 canary: `make smoke-test HOST=hlc-501 IP=10.23.50.51`; verify default bash prompt

**Checkpoint — A0 exit gate**: Both canaries green. If hlc-401 unreachable: check DHCP MAC reservation in Unifi, confirm bcm2711 board (not bcm2712), try different SD card.

---

## Phase 3: US1 — Provision Cluster Nodes (P1) 🎯 MVP

**Goal**: All 9 work-set nodes booted, SSH reachable, standard tools installed, and both update paths validated.
...
- [ ] T030 [US1] Repeat forward+rollback validation on hlc-401 (Pi4)
...
- [ ] T032 [US1] Rewrite `specs/001-nixos-rpi-cluster/quickstart.md`: SD flash → boot → smoke → canary → roll; workstation push as primary path; on-device fallback with caveats (Pi4 may OOM); kitty TERM workaround; direct SSH instructions

**Checkpoint — A2 exit gate**: At least one Pi4 and one Pi5 complete forward+rollback cycle; on-device attempt documented.
...
- [ ] T035 [US1] Roll static IP to `hosts/hlc-502` through `hosts/hlc-508` one at a time: `make canary HOST=hlc-50X IP=10.23.50.5X`; `make smoke-test` after each
...
**Goal**: All 9 work-set nodes have parameterized user module, syshelp, styled PS1, and MOTD via shared modules. Principle III deferral (W-001) retired.

**Independent Test**: SSH into any HLC node as `bob`; MOTD shows HLC ASCII art + hostname + Bob Ross quote; `syshelp` lists installed utilities; PS1 renders without escape artifacts.
...
- [ ] T038 [US4] Verify/create `modules/shell/utilities.nix` with alphabetically sorted packages (git, htop, jq, ripgrep, tmux) + `pkgs.writeShellScriptBin "syshelp"` script; remove inline `environment.systemPackages` from each host config; canary hlc-501; roll to all 9
- [ ] T039 [US4] Verify/create `modules/shell/common.nix` (bash history settings, aliases); canary hlc-501; roll to all 9
- [ ] T040 [US4] Fix `modules/shell/prompt.nix`: replace all `\033[` with `\e[` in color variable definitions; remove wrapping double-quotes from color variable values (see research.md R-011); attempt `nixos-rebuild build-vm` if aarch64 VM is feasible; `make build HOST=hlc-501` → `make canary HOST=hlc-501 IP=10.23.50.51` → explicit interactive PTY SSH to verify no escape artifacts → roll to all 9
- [ ] T041 [US4] Verify/create `modules/motd/default.nix` (parameterized with `environment.etc."motd".text`); verify/create `modules/cluster/hlc/motd.nix` (HLC ASCII art + hostname + Bob Ross quote replicating Debian MOTD); canary hlc-501; roll to all 9; verify MOTD on login
...
- [ ] T043 [US4] Mark W-001 (Principle III deferral) resolved in `WORKAROUNDS.md`; run `make dry-run-all` to confirm all 12 hosts still evaluate clean
...
**Goal**: ecto-1 stub validates the extensible module structure; no existing files modified.
...
- [ ] T044 [P] [US3] Create `modules/cluster/ecto1/` stub with `default.nix` placeholder; parameterize `slimer` user; generic hostname-only MOTD banner
- [ ] T045 [US3] Create `hosts/ecto1-001/configuration.nix` importing shared modules (users/operator.nix with username=slimer, ecto1 MOTD stub); add to `flake.nix` nixosConfigurations; `make dry-run HOST=ecto1-001` must pass with no changes to existing files
...
**Goal**: Rolling update procedure documented and `update-cluster` target verified.

**Independent Test**: `nix flake update` → `make dry-run-all` passes → roll one Pi4 and one Pi5 via `make update-node`; cluster quorum maintained.
...
> Plan these at next `/speckit-plan` invocation after Phase C exits green.
...
- sops-nix: admin age key on gibson; `.sops.yaml` with all 12 recipients; `secrets/hlc.yaml` k3s token
...
- Longhorn with NixOS-compatible images (research.md R-006)
...
T037 → T038 → T039 → T040 → T041 → T042 → T043 — strictly sequential, one module per canary cycle.
...
6. **Stop**: Cluster baseline established. Phases D/E can begin after planning.
...
- T002 (silicon PS1 capture for R-012) must be done before T040 (prompt.nix fix)
