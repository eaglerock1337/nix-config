...
- [ ] T050 Commit all device-ID updates and provisioning-validated configs. Tag `phase5-provisioned`. **Exception**: nodes currently in `DECOM_HOSTS` (hlc-503, hlc-507) are excluded from this tag — tag is valid once all available nodes are provisioned; deferred nodes provisioned as hardware issues clear (tracked in Makefile `DECOM_HOSTS`).
