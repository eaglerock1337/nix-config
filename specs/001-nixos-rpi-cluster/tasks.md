...
- [ ] T091 [US4] Wire `home-manager.users.bob = import ./home/bob.nix` into `mkHlcNode` (path is relative to `flake.nix` since `mkHlcNode` is defined there; the helper already brings in `home-manager.nixosModules.home-manager` per T009; routing the user mapping through `mkHlcNode` keeps per-host configs free of home-manager wiring). Per-host configs do not import `home/bob.nix` directly
