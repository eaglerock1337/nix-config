...
- [X] T033 Add `make provision HOST=<host> [IP=<ip>]` to `Makefile`: resolves IP via HLC convention unless overridden; runs `nixos-anywhere --flake .#<host> --target-host root@<IP> --disko-mode disko --phases disko,install,reboot` (W-010: root@ required — kexec fails on Pi vendor kernel; `--phases` skips kexec path); refuses decom-set hosts (`hlc-402..404`) with message: `"ERROR: hlc-40[234] is decommissioned-set; provision blocked per Constitution cluster-topology rules"`.
