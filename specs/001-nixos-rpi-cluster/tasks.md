...
- [ ] T079 [P] [US4] Create `modules/motd/default.nix` per FR-018: parameterized banner module (option `hlc.motd.banner` for the multi-line ASCII string, option `hlc.motd.quote` for the quote, option `hlc.motd.hostnameLine` defaulting to `Cluster node: <fqdn>`). The module is generic and cluster-agnostic — banner content is injected by the cluster scope, not hard-coded here. Pure Nix-rendered; hostname interpolated from `config.networking.fqdn`
- [ ] T080 [P] [US4] Create `modules/cluster/hlc/motd-banner.nix` as a separate file (NOT folded into `modules/cluster/hlc/default.nix`) so a future cluster scope (e.g. `modules/cluster/ecto/motd-banner.nix`) can supply its own banner without touching the shared MOTD module. Sets `hlc.motd.banner` to the exact post-mortem HLC ASCII art and `hlc.motd.quote` to "Let's build just a happy little cloud." — ~ Bob Ross. Imported from `modules/cluster/hlc/default.nix`
