...
- [ ] T009 In `flake.nix`, re-add the `mkHlcNode` helper that wraps `nixpkgs.lib.nixosSystem` with: the **cluster-common** nvmd modules (sd-image baseline + raspberry-pi base — the modules every Pi node needs regardless of family), `home-manager.nixosModules.home-manager` (with `home-manager.users.bob` wired in T091), and a per-host `configuration.nix`. **Pi-family-specific modules** (`nixos-hardware.nixosModules.raspberry-pi-{4,5}` + nvmd Pi-family overlays) belong in `modules/hardware/rpi{4,5}.nix` (T026/T027), NOT in `mkHlcNode` — the helper takes the family module as an argument from the per-host caller. For now its only caller is `hlc-501` (Pi 5)
...
- [ ] T024 [P] [US2] Create `modules/cluster/common.nix` skeleton: declare a baseline-toolbox placeholder package set inline (a duplicate of the recovery utilities by name only — DO NOT `imports = [ modules/sd/recovery-utils.nix ]`, since the SD bootstrap is by design separate from per-host configs per FR-003; replicating the package list inline is the correct way to give cluster nodes the same recovery utilities pre-toolbox without coupling the SD layer). The placeholder is replaced by `modules/shell/utilities.nix` in Phase 6 / US4. Also include: base sshd posture, bash as canonical shell. NO MOTD wiring here (the MOTD module + option declarations + banner content all land together in Phase 6 / US4 via T079/T080 to avoid declaring options against a not-yet-created module). NO custom PS1 (added in US4)
- [ ] T025 [P] [US2] Create `modules/cluster/hlc/default.nix`: sets FQDN domain to `marks.dev`, sets per-cluster DNS pointers (PiHole, Dream Machine fallback), declares the `bob` operator user identity (the actual operator user *module* lands in US4). NO MOTD content here — banner + quote land in Phase 6 / US4 via T080's separate `motd-banner.nix` file
- [ ] T026 [P] [US2] Create `modules/hardware/rpi4.nix`: Pi-4-specific NixOS hardware module imports (`nixos-hardware.nixosModules.raspberry-pi-4` + nvmd Pi-4 module if separate); force-disable `boot.loader.generic-extlinux-compatible.enable`; **thermal/overclock settings per FR-027 + R-012** — modest overclock matching passive heatsink: `over_voltage=2`, `arm_freq=1750` (the validated Pi 4 profile; stock 1.5 GHz is the documented fallback if thermal validation T119 shows throttling)
- [ ] T027 [P] [US2] Create `modules/hardware/rpi5.nix`: Pi-5-specific module imports (`nixos-hardware.nixosModules.raspberry-pi-5` + nvmd Pi-5 module); NVMe overlay (`dtparam=nvme`); **thermal/overclock settings per FR-027 + R-012** — stock clocks (2.4 GHz; no overclock applied) with the official Active Cooler relied on for kernel-controlled fan curve via standard thermal trip points; no `config.txt` fan-curve override unless a node demonstrates inadequate cooling
- [ ] T028 [P] [US2] Create `modules/cluster/config-txt.nix` (or extend `modules/cluster/common.nix`) with the headless-server `config.txt` profile per [research.md R-011](./research.md) and FR-025: `gpu_mem=16`, `dtparam=audio=off`, `dtoverlay=disable-bt`, `disable_splash=1`, `boot_delay=0`. Pi-family-specific thermal settings live in T026/T027; this task covers only the cluster-common headless profile
...
**Goal**: Toolbox, bash baseline, two-form PS1 (local + remote), HLC MOTD, operator user, modular home-manager, and SSH hardening — each module reintroduced one at a time per W-001's exit plan. Per-module rollout: `make update-node HOST=hlc-501` → `make smoke-test HOST=hlc-501` → on green, serially repeat for `hlc-502..508` then `hlc-401`. On smoke-test failure: `make rollback HOST=<host>`, diagnose, fix, retry. Operator UX is consistent across silicon (workstation) and the cluster (server) per FR-015..FR-020 and SC-006.

This phase opens with adding the `make update-node` and `make rollback` targets — they don't exist yet; this is the first phase that does live-node rollouts post-provisioning. See [contracts/makefile-targets.md](./contracts/makefile-targets.md) §"Added Phase 6 US4".

**Independent Test**: SSH as `bob` into any cluster node — HLC MOTD shows, two-line box-drawing PS1 with `☁⛰☁` glyphs, all toolbox commands resolvable. SSH as `eaglerock` into `silicon` — same toolbox, no HLC MOTD, silicon's local PS1 with `☁⛰☁` substituting `////`. Workstation-only modules (i3, polybar, etc.) apply only to silicon.

### Implementation for User Story 4 — module-by-module per W-001

- [ ] T067 [US4] Create `modules/shell/utilities.nix` with the alphabetical sysadmin toolbox per FR-015 (extends the recovery set): each package on its own line with an inline rationale comment. Imported by `modules/cluster/common.nix` (replacing the recovery-set placeholder from T024)
- [ ] T068 [US4] Update `hosts/silicon/configuration.nix` to also import `modules/shell/utilities.nix` (toolbox is shared with workstation per FR-015)
- [ ] T069 [US4] Add `make update-node HOST=<host>` and `make rollback HOST=<host>` Makefile targets per [contracts/makefile-targets.md](./contracts/makefile-targets.md) §"Added Phase 6 US4". They wrap `sudo nixos-rebuild switch --flake .#<host> --target-host bob@<ip> --use-remote-sudo` and `sudo nixos-rebuild --rollback --flake .#<host> --target-host bob@<ip> --use-remote-sudo` respectively, with HOST→IP derivation. **Both targets MUST refuse decom-set hosts (`hlc-402`, `hlc-403`, `hlc-404`)** per the contract — same gating as `provision` (T051). Then apply the toolbox change to `hlc-501`: `make update-node HOST=hlc-501` → `make smoke-test HOST=hlc-501` (must be green); manually SSH and run `which htop && which jq && which mdadm` to confirm presence. On smoke-test failure: `make rollback HOST=hlc-501`, diagnose, fix, retry
- [ ] T070 [US4] Roll toolbox to remaining Pi 5 work-set: for each of `hlc-502..508`, run `make update-node HOST=<host>` then `make smoke-test HOST=<host>`; on smoke-test failure, `make rollback`, diagnose, fix, retry before moving on
- [ ] T071 [US4] Roll toolbox to `hlc-401`: `make update-node HOST=hlc-401` then `make smoke-test HOST=hlc-401`
- [ ] T072 [US4] Apply toolbox to `silicon`: `make silicon-switch`; manually verify same commands resolvable
- [ ] T073 [P] [US4] Create `modules/shell/common.nix`: bash baseline (canonical shell, baseline aliases, no PS1 setting). Imported by `modules/cluster/common.nix`
- [ ] T074 [US4] Apply common.nix to `hlc-501`: `make update-node HOST=hlc-501` + `make smoke-test`. On green, roll serially to `hlc-502..508` then `hlc-401` (per-host `update-node` + `smoke-test`); also apply to silicon via `make silicon-switch`
- [ ] T075 [US4] Create `modules/shell/prompt.nix` per FR-017 + R-002: local form (silicon-style structure with `☁⛰☁` substituting `////`, color preserved), remote form (two-line box per `remote-ps1.txt`, no color, FQDN). Add `hlc.prompt.mountainGlyph` NixOS option (default: `⛰` + variation selector U+FE0E; fallback: `▲`)
- [ ] T076 [US4] Apply prompt.nix to `hlc-501`: `make update-node HOST=hlc-501` + `make smoke-test`. Then SSH from gibson (kitty terminal, expect remote two-line box), SSH with `TERM=xterm` from a fresh shell (expect remote form rendering correctly without color), and manually log in at the physical console (expect local form). Verify mountain glyph renders cleanly in all three; if not, flip `hlc.prompt.mountainGlyph` to ASCII fallback per-host
- [ ] T077 [US4] Roll prompt to remaining work-set: serially run `make update-node HOST=<host>` + `make smoke-test HOST=<host>` for each of `hlc-502..508` and `hlc-401`
- [ ] T078 [US4] Apply prompt to `silicon` (local form only — silicon never SSHes into itself)
- [ ] T079 [P] [US4] Create `modules/motd/default.nix` per FR-018: parameterized banner module declaring options under a cluster-neutral namespace (`cluster.motd.banner` for the multi-line ASCII string, `cluster.motd.quote` for the quote, `cluster.motd.hostnameLine` defaulting to `Cluster node: <fqdn>`). The module is generic and cluster-agnostic — banner content is injected by the cluster scope, not hard-coded here. The `cluster.*` namespace lets future cluster scopes (e.g. ecto-1) set the same option to their own values without forking the module. Pure Nix-rendered; hostname interpolated from `config.networking.fqdn`. Imported from `modules/cluster/common.nix`
- [ ] T080 [P] [US4] Create `modules/cluster/hlc/motd-banner.nix` as a separate file (NOT folded into `modules/cluster/hlc/default.nix`) so a future cluster scope (e.g. `modules/cluster/ecto/motd-banner.nix`) can supply its own banner without touching the shared MOTD module. Sets `cluster.motd.banner` to the exact post-mortem HLC ASCII art and `cluster.motd.quote` to "Let's build just a happy little cloud." — ~ Bob Ross. Imported from `modules/cluster/hlc/default.nix`
- [ ] T081 [US4] Apply MOTD to `hlc-501`: `make update-node HOST=hlc-501` + `make smoke-test`. SSH as `bob`, observe MOTD matches the post-mortem reference exactly (banner + `Cluster node: hlc-501.marks.dev` + Bob Ross quote). On green, roll serially to `hlc-502..508` then `hlc-401`
- [ ] T082 [US4] Roll MOTD to remaining work-set; smoke-test + spot-check MOTD on each
- [ ] T083 [US4] Create `modules/users/operator.nix`: `bob` user with `operatorPubkey` as authorized key, member of `wheel` and `docker` groups, passwordless wheel (W-002 site annotated with link to `WORKAROUNDS.md`), bash as login shell. Parameterizable so a future `slimer` user can be added without refactor (per FR-013 / spec Out of Scope)
- [ ] T084 [US4] Move `bob`'s definition out of inline host configs (W-001 site removal) into `modules/users/operator.nix`; imported via `modules/cluster/hlc/default.nix`. **T083 + T084 MUST land in a single closure realization** (one `make update-node` switch in T085) — staging across two switches would leave an intermediate generation with no `bob` user, locking the operator out
- [ ] T085 [US4] Apply operator module to `hlc-501` (single switch covering both T083 and T084): `make update-node HOST=hlc-501` + `make smoke-test`. On green, roll serially to `hlc-502..508` then `hlc-401`
- [ ] T086 [P] [US4] Refactor existing `modules/home/base.nix` (present post-Phase-0 reset, inherited from `main`) per [research.md R-009](./research.md). Audit current content first: anything workstation-only (browser config, GUI dotfiles, etc.) moves to `modules/home/workstation.nix` (T088); anything genuinely cross-cutting (neovim baseline, git config with operator's user/email, bash dotfiles, shared aliases) stays in `base.nix`. If `base.nix` is essentially empty / a thin import shim today, expand it per R-009 instead
- [ ] T087 [P] [US4] Create `modules/home/server.nix`: server-only home-manager additions (tmux config tuned for headless work, kubectl/k9s aliases). Imports `modules/home/base.nix`
- [ ] T088 [P] [US4] Create `modules/home/workstation.nix`: imports `modules/home/base.nix` plus existing workstation-only modules (`modules/home/i3.nix`, `polybar.nix`, `dunst.nix`, `dev.nix`, `ui.nix`, `vscode.nix`). `dev.nix` is workstation-only — it pulls in IDE/desktop dev tooling that has no place on a headless cluster node
- [ ] T089 [P] [US4] Create `home/bob.nix` composing `modules/home/base.nix` + `modules/home/server.nix`
- [ ] T090 [US4] Refactor `home/eaglerock.nix` to compose `modules/home/base.nix` + `modules/home/workstation.nix` (remove direct imports of i3/polybar/etc. — those now live behind workstation.nix)
- [ ] T091 [US4] Wire `home-manager.users.bob = import ../../home/bob.nix` into `mkHlcNode` (the helper already brings in `home-manager.nixosModules.home-manager` per T009; routing the user mapping through `mkHlcNode` keeps per-host configs free of home-manager wiring). Per-host configs do not import `home/bob.nix` directly
- [ ] T092 [US4] Apply home-manager refactor to `silicon`: `make silicon-switch`; verify i3/polybar still working (workstation profile)
- [ ] T093 [US4] Apply home-manager refactor to `hlc-501` (server profile): `make update-node HOST=hlc-501` + `make smoke-test`. SSH as `bob`, verify neovim opens with baseline config, `git --version` works, tmux config loaded
- [ ] T094 [US4] Roll home-manager to remaining work-set
- [ ] T095 [US4] **SSH hardening** (closes W-003): in `modules/cluster/common.nix`, set `services.openssh.settings.PasswordAuthentication = false` and `services.openssh.settings.KbdInteractiveAuthentication = false`. Update FR-020 site annotation
- [ ] T096 [US4] Apply SSH hardening to `hlc-501`: `make update-node HOST=hlc-501` + `make smoke-test` (key-only login still works); manually attempt password auth from gibson and confirm rejection
- [ ] T097 [US4] Roll SSH hardening to remaining work-set; smoke-test after each
- [ ] T098 [US4] Update `WORKAROUNDS.md`: mark W-001 Resolved (date YYYY-MM-DD) — all six modules reintroduced via canary; mark W-003 Resolved (date YYYY-MM-DD) — SSH hardening live on all 9 work-set nodes. W-002 stays open (deferred to future secrets-mgmt feature spec)
...
- [ ] T100 [US5] Create `modules/k8s/prereqs.nix` per [research.md R-003](./research.md): `services.k3s.enable = true`, `systemd.services.k3s.wantedBy = lib.mkForce [ ]`, container runtime deps (`containerd`, `runc`), `pkgs.k9s`, shell alias `k = kubectl` in the bash baseline, kernel modules (`br_netfilter`, `overlay`), sysctls (`net.bridge.bridge-nf-call-iptables = 1`, `net.ipv4.ip_forward = 1`, etc.) per upstream k3s requirements
...
- **Within US4**: module *creation* tasks (T067, T073, T075, T079, T086..T089) are parallel-safe (different files). *Deployment* tasks (canary + roll) are serial by safety design (Constitution IV — one node at a time).
...
3. Implement the tasks in this list for that phase.
4. **Canary on the designated node** (`hlc-501` for cluster work; silicon for workstation work).
