...
3. **PS1** (`modules/cluster/prompt.nix`) — generic cluster-tier mechanism. Option `cluster.prompt.glyph` (default `"-"`; generic fallback). HLC layers `hlc.prompt.glyph` option (default `"☁️🏔️☁️"` emoji-presentation; text-presentation fallback `"☁⛰︎☁"`, R-002) in `modules/cluster/hlc/default.nix` and wires `cluster.prompt.glyph = config.hlc.prompt.glyph`. Local form: silicon-style, Gruvbox colors, `////` → cluster glyph. Remote form: two-line box-drawing (`┌─╸user@fqdn <glyph> [cwd]` / `└──╸$`), no color codes, FQDN. Validate rendering on kitty + `xterm-256color` SSH + `TERM=xterm` SSH. Workstation does NOT import this module — silicon PS1 unchanged.
