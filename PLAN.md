...
- **4x RPi 4** (hlc-401 to hlc-404): k3s server role — etcd, ingress, general workloads
...
│   ├── rpi4.nix                  # RPi 4: board bcm2711, networking, stateVersion
│   ├── rpi5.nix                  # RPi 5: board bcm2712, NVMe, stateVersion
│   └── disko/
...
- Neovim with gruvbox (same as base.nix but without GUI plugins)
- Bash with useful aliases: `ll`, `la`, `k` (kubectl), `kns`, `kctx`, `h` (helm)
...
services.k3s = {
  ...
  # clusterInit and serverAddr set per-host
};
...
- Comment: Longhorn requires NixOS-compatible container images — override via ArgoCD Helm values
...
sops.defaultSopsFile = ../../secrets/hlc-401.yaml;
```
...
services.k3s.serverAddr = "https://hlc-401:6443";
sops.defaultSopsFile = ../../secrets/hlc-5XX.yaml;
```
...
- `secrets/` directory with one encrypted YAML per node
- Each file contains `k3s-token` (same value all nodes)
- `.sops.yaml` at repo root: admin age key + per-node host ed25519 key as recipients
- Host keys added to `.sops.yaml` after first SD boot, secrets re-encrypted

User setup steps (one-time):
1. `age-keygen -o ~/.config/sops/age/keys.txt`
2. Add pubkey to `.sops.yaml` admin recipients
3. `sops secrets/hlc-401.yaml` — add k3s-token
4. After provisioning each node: get host pubkey, re-run `sops update-keys secrets/hlc-N.yaml`
...
1. hlc-401 (clusterInit)
2. hlc-402, 403, 404 (join etcd)
3. hlc-501 through hlc-508 (join as agents)
4. ArgoCD bootstrap via kubectl
...
update-cluster-pi4:     update all hlc-4XX nodes sequentially
update-cluster-pi5:     update all hlc-5XX nodes sequentially
update-cluster:         update all 12 nodes (Pi 4 then Pi 5)
...
| Create | `.sops.yaml` |
| Create | `secrets/` (gitignored plaintext, tracked encrypted) |
| Modify | `Makefile` |
...
- **Phase 3**: Longhorn prerequisites → ArgoCD deploys Longhorn with NixOS images
